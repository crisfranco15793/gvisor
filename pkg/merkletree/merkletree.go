...
// restored.
func Generate(data io.ReadSeeker, dataSize int64, treeReader io.ReadSeeker, treeWriteSeeker io.WriteSeeker, dataAndTreeInSameFile bool) ([]byte, error) {
	...
	if dataAndTreeInSameFile && bytesInLastBlock != 0 {
		...
		if _, err := treeWriteSeeker.Seek(0, io.SeekEnd); err != nil && err != io.EOF {
			return nil, err
		}
		if _, err := treeWriteSeeker.Write(zeroBuf); err != nil {
			return nil, err
		}
	}
	...
	for level := 0; level < layout.numLevels(); level++ {
		for i := int64(0); i < numBlocks; i++ {
			...
			if _, err = treeWriteSeeker.Write(digest[:]); err != nil {
				return nil, err
			}
		}
